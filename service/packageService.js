...
const packageService = {
  ...
  addOrUpdatePackage: async (data, file) => {
    ...
    if (!mongoose.Types.ObjectId.isValid(className)) {
      throw new Error('Invalid className ID');
    }

    const classExists = await ClassMaster.findById(className);
    if (!classExists) {
      throw new Error('Class not found in ClassMaster');
    }
